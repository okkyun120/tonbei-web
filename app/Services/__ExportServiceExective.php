...
class ExportServiceExective
{
    public function makePdf($file_name, $id)
    {
        ...
        $invetmentData = TBPC003Model::showInvestment($id);     // 出資情報

        // 項目取り出し

        // メイン情報
        foreach ($mainData as $data) {
            $type_name_array[] = $data->type_name;                      // 実施形態
...
/*
        // 関係先データ編集
        for ($i = 0; $i < count($relation_title_array); $i++) {
            $output_relation_title .= $relation_title_array[$i];
            $output_relation_parties .= $related_parties_array[$i];
        }
*/
